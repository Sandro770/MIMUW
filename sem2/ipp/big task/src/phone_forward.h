 ...
 * @date 2022
 *
 * Modified by Alessandro Peria <alessandroperia@icloud.com>
 */
 ...
 */
struct PhoneForward;
typedef struct PhoneForward PhoneForward; /**< lukier syntaktyczny */

/**
 * To jest struktura przechowująca ciąg numerów telefonów.
 */
struct PhoneNumbers;
typedef struct PhoneNumbers PhoneNumbers; /**< lukier syntaktyczny */
 ...
 * Wyznacza następujący ciąg numerów: jeśli istnieje numer @p x, taki że jego
 * przekierowanie jest numerem @p num to numer @p x należy do wyniku wywołania
 * @ref phfwdReverse z numerem @p num. Przekierowanie w definicji wyżej
 * bierze pod uwagę niekoniecznie najdłuższy pasujący prefiks. Dodatkowo ciąg
...
/** @brief Wyznacza przekierowania na numer.
 * Dla podanej za pomocą wskaźnika pf bazy przekierowań i podanego numeru
 * telefonu num wyznacza posortowaną leksykograficznie listę wszystkich
 * takich numerów telefonów i tylko takich numerów telefonów x,że
 * phfwdGet(x) = num. Funkcja ta alokuje strukturę PhoneNumbers, która musi
 * być zwolniona za pomocą funkcji phnumDelete.
 * @param pf - wskaźnik na strukturę przechowującą przekierowania numerów;
 * @param num - wskaźnik na napis reprezentujący numer.
 * @return Wskaźnik na strukturę przechowującą ciąg numerów lub pusty ciąg
 * jeśli podany napis nie reprezentuje numeru, lub NULL, gdy nie udało się
 * alokować pamięci.
 */
